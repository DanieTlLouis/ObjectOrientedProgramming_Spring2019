...
book*
// my_min_element(book* first, book* last, comparison comp)
my_min_element(book* first, book* last, bool(*comp)(book const&, book const&))
{
  assert(first != last);
  book* min = first++;
...
insertion_sort(book* first, book* last, bool(*comp)(book const&, book const&))
{
  while (first != last) {
...
{
  std::vector<book> v {
    {"The Lord or the Rings", "Tolkien", "1954"},
  ...
  book* first = v.data();
  book* last = first + v.size();

  // std::cout << "BEFORE\n";
  // print(v);

  // insertion_sort(v.data(), v.data() + v.size(), operator<);
  // std::cout << "LESS\n";
  // print(v);
  // insertion_sort(v.data(), v.data() + v.size(), operator>);
  // std::cout << "GREATER\n";
  ...
  // insertion_sort(v.data(), v.data() + v.size(), author_year_asc);
...
#if 0
  // This is the "closure type" of the lambda
  // expression above.
  struct __some_name
  {
    __some_name(int asc)
    ...
    auto operator()(book const& a, book const& b) const
    {
      if (asc)
        return a.year < b.year;
      else
  ...
  insertion_sort(first, last, __some_name(asc));
  print(v);
#endif
}
